...
def test_auth_flow(client, mock_google_client, db):
    with client:
...
def test_revoke_credentials_successful(client, db, mock_post,
                                       mock_google_oauth2):
    creds = json.dumps({
        'token': '123',
        'refresh_token': '456',
...
def test_revoke_credentials_invalid_guild_id(client, mock_post):
    response = client.get('/revoke/123', headers={'Authorization': 'key'})
    assert response.status_code == 400
    assert response.text == 'bad request'


def test_revoke_credentials_server_error(client, db, mock_post):
    creds = json.dumps({
        'token': '123',
        'refresh_token': '456',
        'token_uri': 'https://accounts.google.com/o/oauth2/token',
...
def test_revoke_credentials_valid_guild_id_no_credential(client, db):
    db.execute(text("INSERT INTO guild VALUES ('123', 'null', 'xyz')"))
    db.commit()
    response = client.get('/revoke/123', headers={'Authorization': 'key'})
    assert response.status_code == 400

def test_revoke_credentials_no_api_key(client):
    response = client.get('/revoke/123')
    assert response.status_code == 403

def test_revoke_credentials_invalid_api_key(client):
